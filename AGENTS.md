...
- **Native iOS app (`ios/`):** Swift/SwiftUI app targeting iOS 16+. Requires macOS with Xcode 15+ and [XcodeGen](https://github.com/yonaskolb/XcodeGen). Run `xcodegen generate` in `ios/` to create `LeaveANote.xcodeproj`, then open in Xcode. Firebase iOS SDK added via SPM. Replace `GoogleService-Info.plist` with your own from Firebase Console. See `ios/README.md` for full setup.
